...
switch ($method) {
    ...
    if (!$data || !isset($data['subject'], $data['date'], $data['status'])) {
        http_response_code(400);
        echo json_encode(array("success" => false, "message" => "Datos incompletos o inválidos"));
        break;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO asistencias (subject, date, status) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $subject, $date, $status);

    if ($stmt->execute()) {
        echo json_encode(array("success" => true, "message" => "Asistencia agregada con éxito"));
    } else {
        http_response_code(500);
        echo json_encode(array("success" => false, "message" => "Error al agregar asistencia: " . $stmt->error));
    }
    $stmt->close();
    break;
