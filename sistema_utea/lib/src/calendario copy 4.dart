...
class Evento {
  ...
  factory Evento.fromJson(Map<String, dynamic> json) {
    // Manejo más seguro de color
    int colorValue = 0xFF2196F3; // color default azul
    if (json['color'] != null) {
      if (json['color'] is int) {
        colorValue = json['color'];
      } else if (json['color'] is String) {
        colorValue = int.tryParse(json['color']) ?? colorValue;
      }
    }
    return Evento(
      ...
      fechaFin: DateTime.tryParse(json['fechaFin'] ?? '') ?? DateTime.now().add(const Duration(hours: 1)),
