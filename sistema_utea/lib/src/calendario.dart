...
class Evento {
  ...
  factory Evento.fromJson(Map<String, dynamic> json) {
    ...
    return Evento(
      ...
      fechaInicio: DateTime.tryParse(json['start_date'] ?? '') ?? DateTime.now(),
      fechaFin: DateTime.tryParse(json['end_date'] ?? '') ?? DateTime.now().add(const Duration(hours: 1)),
      ...
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Colors.blue.shade200, Colors.white], // Gradiente similar a la imagen
      ...
      content: Wrap(
        ...
        children: gradientOptions.map((gradient) {
          return GestureDetector(
            onTap: () => setState(() => pickedColor = gradient[0]),
            child: Container(
              width: 36,
              height: 36,
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: pickedColor == gradient[0] ? Colors.black : Colors.transparent,
                  width: 2,
                ),
              ),
            ),
