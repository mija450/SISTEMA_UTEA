...
class _CalendarioScreenState extends State<CalendarioScreen> {
  ...
  Future<void> _loadEvents() async {
    ...
    final url = Uri.parse("http://127.0.0.1/ProyectoColegio/Sistema_Utea/calendario.php");
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          _showError("Formato de respuesta inesperado");
        }
      } else {
        _showError("Error al obtener eventos del calendario");
      }
    } catch (e) {
      _showError("Error: $e");
    } finally {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildColorOption("Azul Claro", Colors.lightBlue.shade100),
              ],
            ),
          ),
        );
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Fecha de Inicio (yyyy-MM-dd HH:mm:ss)'),
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Fecha de Fin (yyyy-MM-dd HH:mm:ss)'),
                    validator: (value) {
                      if (value == null || value.isEmpty) {
  ...
  void _saveEvent() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        _loadEvents();
        _showSnackBar("Evento guardado con éxito");
      } else {
        _showSnackBar("Error al guardar el evento");
      }
    } catch (e) {
  ...
  void _deleteEvent(int index) async {
    ...
    try {
      final response = await http.delete(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'id': event['id']}),
      );

      if (response.statusCode == 200) {
        _loadEvents();
        _showSnackBar("Evento eliminado con éxito");
      } else {
  ...
  void _updateEvent(int index) async {
    final url = Uri.parse("http://127.0.0.1/ProyectoColegio/Sistema_Utea/calendario.php");

    final Map<String, dynamic> eventData = {
      'id': events[index]['id'], // Assuming the API needs the ID to update
      'title': _titleController.text,
      'start_date': _startDateController.text,
      'end_date': _endDateController.text,
      'description': _descriptionController.text,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackBar("Error al actualizar el evento");
      }
    } catch (e) {
      _showSnackBar("Error: $e");
    }
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : events.isEmpty
                      ? const Center(child: Text('No hay eventos programados', style: TextStyle(fontSize: 16)))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Dismissible(
                              ...
                              child: _buildEvent(event['title'], event['start_date'], event['end_date'], event['description'], index),
                            );
                          },
                        ),
            ),
  ...
  Widget _buildEvent(String title, String startDate, String endDate, String description, int index) {
    return GestureDetector(
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Abrir detalles de: $title')));
      },
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(title, style: TextStyle(color: primaryColor, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(DateFormat('dd/MM/yyyy hh:mm a').format(DateTime.parse(startDate)), style: const TextStyle(color: Colors.grey)),
                Text(DateFormat('dd/MM/yyyy hh:mm a').format(DateTime.parse(endDate)), style: const TextStyle(color: Colors.grey)),
              ],
            ),
