...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildLogo() {
    // Si no tienes logo blanco, puedes usar un texto blanco estilizado aquí.
    return Column(
      children: [
        // Si tienes imagen en blanco:
        // CircleAvatar(
        //   backgroundImage: AssetImage("assets/images/logo_utea_blanco.png"),
        //   radius: 50,
        //   backgroundColor: Colors.transparent,
        // ),

        // Alternativa texto blanco con estilo UTEA
        Text(
          "UTEA",
...
class __AnimatedBubbleBackgroundState extends State<_AnimatedBubbleBackground> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(vsync: this, duration: Duration(seconds: 40))
      ..addListener(() {
        setState(() {});
      })
      ..repeat();
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Color(0xFF1B2A49), Color(0xFF244D6A)], // tonos azulados más suaves
...
class _Bubble {
  ...
  _Bubble(this.random, this.index) {
  ...
  Offset getPosition(double progress) {
    double y = (position.dy + progress * speed * 60 + index * 0.02) % 1;
...
class _BubblePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var bubble in bubbles) {
      final pos = bubble.getPosition(progress);
      final Offset offset = Offset(pos.dx * size.width, pos.dy * size.height);
