...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          AnimatedSparkleBackground(), // Cambia aquí el fondo
          Center(
...
class AnimatedSparkleBackground extends StatefulWidget {
  const AnimatedSparkleBackground({Key? key}) : super(key: key);

  @override
  State<AnimatedSparkleBackground> createState() => _AnimatedSparkleBackgroundState();
}

class _AnimatedSparkleBackgroundState extends State<AnimatedSparkleBackground>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  final List<_Sparkle> _sparkles = [];
  final List<_Bubble> _bubbles = [];
  final int numberOfSparkles = 40;
  ...
  return Container(
    decoration: const BoxDecoration(
      gradient: LinearGradient(
        colors: [Color(0xFFF0F0F0), Color(0xFFFFFFFF)], // Fondo claro blanco/gris muy suave
        begin: Alignment.topLeft,
        end: Alignment.bottomRight,
      ),
    ),
    child: CustomPaint(
      painter: _CombinedPainter(_bubbles, _sparkles, _controller.value),
      child: Container(),
      ),
    );
  }
...
// Clase que combina burbujas y chispas
class _CombinedPainter extends CustomPainter {
  final List<_Bubble> bubbles;
  final List<_Sparkle> sparkles;
  final double progress;

  _CombinedPainter(this.bubbles, this.sparkles, this.progress);

  @override
  void paint(Canvas canvas, Size size) {
    final Paint paint = Paint();

    // Pintar burbujas verdes translúcidas
    ...
    // Pintar chispas blancas con parpadeo
    for (var sparkle in sparkles) {
      double opacity = 0.5 + 0.5 * sin(progress * 2 * pi * sparkle.twinkleSpeed + sparkle.twinklePhase);
      paint.color = const Color.fromARGB(255, 255, 255, 255).withOpacity(opacity.clamp(0, 1));
      final Offset pos = Offset(sparkle.x * size.width, sparkle.y * size.height);
      canvas.drawCircle(pos, sparkle.size, paint);
    }
  }
